<?php
    ...
    $id_cus = $_SESSION['id'];
    $query = "SELECT p.foto, p.id, c.id_customers, c.id_produk, p.nama, c.harga_produk, c.banyak_produk, c.total FROM produk p INNER JOIN orders c ON p.id=c.id_produk WHERE c.id_customers=$id_cus AND c.status='cart';";
            ...
            <div class="col-lg-8 table-responsive mb-5">
                <table class="table table-bordered text-center mb-0">
                    ...
                    <tbody class="align-middle">
                        ...
                        <tr>
                            <td class="align-middle"><img src="foto/<?= $r['foto']; ?>" alt="" style="width: 50px;"> <?= $r['nama'] ?></td>
                            <td class="align-middle">Rp.<?= $r['harga_produk'] ?></td>
                            <td class="align-middle">
                                <div class="input-group quantity mx-auto" style="width: 100px;">
                                    <div class="input-group-btn">
                                        <a href="functions/reduce.php?id_produk=<?= $r['id_produk']?>&url=cart&harga_produk=<?= $r['harga_produk'] ?>" class="btn btn-sm btn-warning btn-minus" >
                                            <i class="fa fa-minus"></i>
                                        </a>
                                    </div>
                                    <input type="text" class="form-control form-control-sm bg-secondary text-center" value="<?= $r['banyak_produk']; ?>">
                                    <div class="input-group-btn">
                                        <a href="functions/addToCart.php?id_produk=<?= $r['id_produk']?>&url=cart&harga_produk=<?= $r['harga_produk'] ?>" class="btn btn-sm btn-warning btn-plus">
                            ...
                            <td class="align-middle">
                                <?php
                                    $harga = $r['harga_produk'];
                                    $banyak = $r['banyak_produk'];
                                    $total = $harga * $banyak;
                                    echo "Rp." . $total;
                                ?>
                            </td>
                            <td class="align-middle"><a href="functions/delete.php?id_produk=<?= $r['id_produk'] ?>&banyak=<?= $r['banyak_produk'] ?>&total=<?= $r['total'] ?>" class="btn btn-sm btn-warning"><i class="fa fa-times"></i></a></td>
                        </tr>
                        <?php endforeach; ?>
                ...
                <div class="card border-secondary mb-5">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between mb-3 pt-1">
                            ...
                            <h6 class="font-weight-medium">
                                <?php
                                    $id_cus = $_SESSION['id'];
                                    $query = "SELECT total FROM customers WHERE id=$id_cus AND status_order='cart'";
                                    $result = mysqli_query($conn, $query);
                                    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
                                    if($row)
                                    {
                    ...
                    <div class="card-footer border-secondary bg-transparent">
                        ...
                        <?php
                            ...
                            $result = mysqli_query($conn, $query);
                            $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
                        if($row['status_order'] == 'cart')
                        {
                            echo "<a href='functions/toCheckout.php' class='btn btn-block btn-warning my-3 py-3'>Proceed To Checkout</a>";
                        }
                        ?>
                    </div>
                </div>
